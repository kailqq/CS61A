...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        index_to_del=[]
        for i in range(len(copy_opponent)):
            if copy_opponent[i].attack==copy_opponent[i].defense:
                index_to_del.append(i)
        new_deck=[]
        for i in range(len(copy_opponent)):
            if i not in index_to_del:
                new_deck.append(copy_opponent[i])
        opponent.deck.cards=new_deck 
