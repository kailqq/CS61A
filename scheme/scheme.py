...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if vals is nil:
                raise SchemeError('Oops! too few vals')
            else:
                child.define(formals.first, vals.first)
                formals, vals = formals.rest, vals.rest
        if vals is not nil:
            raise SchemeError('Oops! too many vals')
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('oops, something went wrong: {0}'.format(e))
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        procedure = do_lambda_form(Pair(target.rest, expressions.rest), env)#formal and expressions
        env.define(target.first, procedure)# name of function and its body
        return target.first#return its name
        # END PROBLEM 9
    else:
    ...
    if expressions is nil:
        ...
        while expressions.rest is not nil:
            if is_true_primitive(scheme_eval(expressions.first, env)):
                ...
                return False
        last_value= scheme_eval(expressions.first, env,True)
        if is_true_primitive(last_value):
            return last_value
        else:
            return False
    # END PROBLEM 12
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    def make_call_frame(self, args, env):   #we don't need to change the env anymore 
