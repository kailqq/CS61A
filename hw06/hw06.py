...
def remove_all(link , value):
    ...
    save=link
    look_up=link.rest
    while look_up is not Link.empty:
        if look_up.first!=value:
            save.rest=look_up
            save=save.rest
        look_up=look_up.rest
    save.rest=Link.empty
...
def deep_map(f, link):
    ...
    "*** YOUR CODE HERE ***"
    #from kailqq:
    #we need to create a new link with the same structure as link but 
    # it can't be done by simply use new_link=link
    if link is Link.empty:
        return Link.empty
    new_link=Link(link.first,link.rest)
    if type(new_link.first)==Link:
        new_link.first=deep_map(f,new_link.first)
    else:
        new_link.first=f(new_link.first)
    new_link.rest=deep_map(f,new_link.rest)
    return new_link
