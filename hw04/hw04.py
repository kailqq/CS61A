...
def has_path(t, phrase):
    ...
    if label(t) != phrase[0]:
        ...
        if len(phrase) == 1:
            ...
            return any([has_path(b, phrase[1:]) for b in branches(t)])
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, "AssertionError: y should not span 0"
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= extreme_point <= upper_bound(x):
        upper=max(f(lower_bound(x)), f(upper_bound(x)), f(extreme_point))
        lower=min(f(lower_bound(x)), f(upper_bound(x)), f(extreme_point))
    else:
...
def check_par():
    ...
    True
    """
    r1 = interval(1.5, 1.4) # Replace this line!
    r2 = interval(1.2, 1.6) # Replace this line!
    return r1, r2
