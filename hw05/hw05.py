...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    protect=[]
    def withdraw(number,passw):
        nonlocal balance
        nonlocal password
        if len(protect)==3:
            return "Too many incorrect attempts. Attempts: "+str(protect)
        elif passw!=password:
            protect.append(passw)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(result)==str:
        return result
    def joint(number,passw):
        if passw==new_pass:
            return withdraw(number,old_pass)
        else:
            return withdraw(number,passw)
    return joint
